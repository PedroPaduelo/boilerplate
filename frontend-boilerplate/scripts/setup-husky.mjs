#!/usr/bin/env node
/**
 * Setup Husky hooks for frontend-boilerplate.
 *
 * Why this exists:
 *
 ...
 *
 * What this does:
 *   - Uses a dedicated `frontend-boilerplate/.husky-run/` directory
...
# Pre-commit hook (frontend-boilerplate).
# Invoked by the husky shim via \`core.hooksPath=.husky-run/_/\`. The
# shim's working directory is wherever \`git commit\` was invoked from,
# so we \`cd\` into the package root before running the hook body.
...
# Commit-msg hook (frontend-boilerplate).
# Invoked by the husky shim via \`core.hooksPath=.husky-run/_/\`. The
# shim's working directory is wherever \`git commit\` was invoked from,
# so we \`cd\` into the package root before running the hook body.
SCRIPT_DIR=$(cd "$(dirname "$0")" && pwd)
