 ...
 * valor da série em destaque, tendência desenhada e selo de variação. O cartão
 * é próprio do bloco (`signal-card.tsx`), montado sobre o design system.
 *
 ...
 *  - `accent`: cor da TENDÊNCIA desenhada, resolvida no component.tsx para um
 *    token de dado do design system (pinta traço/preenchimento da série, NUNCA
 *    o fundo do cartão). Default `'chart-1'`.
...
export const manifest = {
  ...
  name: 'Signal Card',
  description:
    'KPI com mini-sparkline e tendência. Recebe uma série; destaca o último valor.',
  source: 'custom',
  propsSchema: {
    type: 'object',
    ...
    properties: {
      label: {
        ...
        default: 'Sinal',
        description:
          'Rótulo curto do sinal exibido acima do valor (ex.: "Latência p95", "Arrecadação").',
      },
      // (valueFormat) ENUM FECHADO — formato PT-BR do valor em destaque.
      ...
      valueFormat: {
        ...
        description:
          'Formato PT-BR do valor em destaque (último ponto da série). ENUM FECHADO (sem input livre). Default "compactNumber".',
        oneOf: [
          {
            const: 'BRL',
          ...
          },
          {
            const: 'compactNumber',
            description:
              'formatCompactNumberBR — número compacto (ex.: "2,61 bi"). DEFAULT.',
          },
          {
            const: 'percent',
            description:
      ...
      // COR DA SÉRIE — enum do catálogo; o componente resolve para token de
      // dado do DS e pinta o traço/preenchimento da tendência, nunca o fundo.
      accent: {
        type: 'string',
        enum: [...ACCENT_COLORS],
        default: 'chart-1',
        description:
          'Cor da tendência desenhada (traço e preenchimento; nunca o fundo do cartão). O valor é resolvido para uma cor de dado do design system (chart-1..5 e primary mapeiam para as cores categóricas, na mesma ordem da paleta). Valores fora do enum são aceitos por compatibilidade e caem na paleta quando não descrevem uma cor do sistema.',
      },
      ...
      trendPolarity: {
        ...
        default: 'up-good',
        description:
          'Direção considerada "boa": "up-good" (subir = verde, padrão) ou "up-bad" (subir = vermelho, ex.: latência/erro).',
      },
      ...
      trendBasis: {
        ...
        description:
          'Base do cálculo da tendência: "prev-vs-last" (default) = último vs penúltimo (variação mais recente); "first-vs-last" = último vs primeiro (variação no período inteiro).',
      },
      ...
      showSparkline: {
        ...
        default: true,
        description:
          'Mostra (true, default) ou esconde (false) a mini-sparkline abaixo do valor.',
      },
    },
  },
  dataContract: {
