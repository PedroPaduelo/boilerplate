/**
 * Manifesto do bloco `metric_glow` (shape 'scalar') — card de métrica em
 * destaque. O cartão é próprio do bloco (`metric-glow-card.tsx`), montado sobre
 * o design system.
 *
 ...
 *  - `accent`: cor do halo de destaque, resolvida em runtime para um token de
...
export const manifest = {
  ...
  name: 'Métrica (glow)',
  description:
    'Card de métrica única em destaque, com variação e halo de luz atrás do número.',
  source: 'custom',
  propsSchema: {
    type: 'object',
    ...
    properties: {
      ...
      valueFormat: {
        ...
        oneOf: [
          {
            const: 'BRL',
            description: 'formatBRL — moeda BRL completa (ex.: "R$ 124.500,00").',
          },
          {
            const: 'compactBRL',
            description:
          ...
          {
            const: 'compactNumber',
            description: 'formatCompactNumberBR — número compacto (ex.: "124,5 mil").',
          },
          {
            const: 'percent',
            description:
      ...
      // e a aplica ao halo de destaque do card.
      accent: {
        type: 'string',
        enum: [...ACCENT_COLORS],
        default: 'chart-1',
      ...
      showDelta: {
        ...
        default: true,
        description:
          'Mostra a variação percentual (%) abaixo do valor. `false` esconde a variação mesmo quando há `delta` nos dados.',
      },
        ...
        oneOf: [
          {
            const: 'up-good',
            description:
              'Subir é bom: variação positiva = verde, negativa = vermelho. DEFAULT (ex.: receita, vendas).',
          },
          {
            const: 'up-bad',
            description:
              'Subir é ruim: variação positiva = vermelho, negativa = verde (ex.: inadimplência, custos, atrasos).',
          },
        ],
      },
    },
