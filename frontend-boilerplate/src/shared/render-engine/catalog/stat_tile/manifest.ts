 ...
 *  - `accent`       (string)  — cor de categorização do ladrilho, resolvida em
 *                               runtime para uma variante de cor do design
 *                               system.
  ...
  propsSchema: {
    ...
    properties: {
      label: {
        type: 'string',
        description:
          'Rótulo do ladrilho. Sobrescreve o `label` derivado dos dados (data.label). Se ausente, usa data.label e, por fim, o nome do bloco.',
      },
      ...
      valueFormat: {
        ...
        default: 'compactNumber',
        description:
          'Formato PT-BR do valor exibido. ENUM FECHADO (sem input livre). Substitui o prefixo de moeda hardcoded.',
        oneOf: [
          {
            const: 'BRL',
      ...
      showDelta: {
        ...
        default: true,
        description:
          'Mostra a variação (delta) quando os dados trazem `delta`. `false` esconde a variação.',
      },
      deltaPolarity: {
        type: 'string',
        enum: ['up-good', 'up-bad'],
        default: 'up-good',
        description:
          'Polaridade da variação: "up-good" (subir é bom → positivo verde) | "up-bad" (subir é ruim → positivo vermelho).',
        oneOf: [
          {
            const: 'up-good',
            description:
              'Subir é bom: delta positivo = verde, negativo = vermelho. DEFAULT.',
          },
          {
            const: 'up-bad',
            description: 'Subir é ruim: delta positivo = vermelho, negativo = verde.',
          },
        ],
      },
      hint: {
