...
Este pacote **não tem script `prepare`** (não rebuilda no install). Por isso o
`dist/` é **versionado** (commitado) — assim `npm install` resolve o pacote sem
um build-step e sem precisar de permissão de escrita aqui. Regenere o `dist`
(`npm run build`) e commite quando alterar os contratos.

> **Use `npm install ../shared/contracts --install-links`** (em vez do symlink
> padrão do `file:`). Com symlink, o Node resolve as deps do pacote (ex.: `ajv`)
> a partir do caminho REAL (`shared/contracts/node_modules`), que não existe →
> `Cannot find module 'ajv'`. `--install-links` instala uma **cópia real** em
> `node_modules/@dashboards/contracts` e as deps resolvem do `node_modules` do
> consumidor (BE/FE), onde o `npm` as instala transitivamente.
